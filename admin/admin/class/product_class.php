...
class product
{
    ...
    public function update_product($data, $file, $sanpham_id)
    {
        ...
        if (!empty($file_name)) {
            ...
            $sanpham_anh = $this->get_current_image($sanpham_id); 
        }
        ...
        $filenames = $file['sanpham_anhs']['name'];
        $filetmps = $file['sanpham_anhs']['tmp_name'];
        if (!empty($filenames[0])) {
            $query = "DELETE FROM tbl_sanpham_anh WHERE sanpham_id = '$sanpham_id'";
            $this->db->delete($query);
            foreach ($filenames as $key => $filename) {
                $file_ext = strtolower(pathinfo($filename, PATHINFO_EXTENSION));
                $sanpham_anh_multi = substr(md5(time() . $key), 0, 10) . '.' . $file_ext;
                $upload_image_multi = "uploads/" . $sanpham_anh_multi;
                move_uploaded_file($filetmps[$key], $upload_image_multi);
                $query = "INSERT INTO tbl_sanpham_anh (sanpham_id, sanpham_anh) VALUES ('$sanpham_id', '$sanpham_anh_multi')";
    ...
    private function get_current_image($sanpham_id)
    {
        $query = "SELECT sanpham_anh FROM tbl_sanpham WHERE sanpham_id = '$sanpham_id'";
        $result = $this->db->select($query);
        if ($result) {
            $data = $result->fetch(PDO::FETCH_ASSOC);
            return $data['sanpham_anh'];
        }
        return null;
    }
