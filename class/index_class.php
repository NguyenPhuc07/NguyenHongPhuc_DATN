...
class index
{
    ...
    public function get_loaisanpham($loaisanpham_id, $order_by = 'sanpham_id ASC')
    {
    ...
    public function insert_order(
        ...
        if ($result) {
            ...
            error_log("Lỗi khi chèn bản ghi: ");
        }

        return $result;
    }

    public function update_su_dung_diem_tich_luy($session_idA, $su_dung_diem_tich_luy)
    {
        $query = "UPDATE tbl_order SET su_dung_diem_tich_luy = ? WHERE session_idA = ?";
        if ($stmt = $this->db->prepare($query)) {
            $stmt->bind_param("is", $su_dung_diem_tich_luy, $session_idA);
            ...
            error_log("Lỗi trong việc chuẩn bị truy vấn UPDATE");
            return false;
        }
    }
        ...
        if ($result == null) {
            ...
            if ($resultA) {
                ...
                while ($resultB = $resultA->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $tong_tien += $sanpham_gia * $quantitys;
                }

                $diem_tich_luy_su_dung = isset($_SESSION['su_dung_diem_tich_luy']) ? $_SESSION['su_dung_diem_tich_luy'] : 0;
                $ap_dung_diem_tich_luy = $diem_tich_luy_su_dung > 0 ? 1 : 0;

                $query = "INSERT INTO tbl_order_moi (session_idA, tong_tien, diem_tich_luy_su_dung, ap_dung_diem_tich_luy, ngay_tao) VALUES 
                  ('$session_idA', $tong_tien, $diem_tich_luy_su_dung, $ap_dung_diem_tich_luy, '$today')";
                $this->db->insert($query);

                // Tính số điểm tích lũy từ tổng tiền đơn hàng
                $so_diem_tich_luy = floor($tong_tien / 10000);

                $user_id = $_SESSION['user_id'];
                $query_get_diem = "SELECT diem_tich_luy FROM khach_hang WHERE id = ?";
                $current_diem_tich_luy = $this->db->select($query_get_diem, [$user_id])->fetchColumn();

                if ($current_diem_tich_luy !== false) {
                    // Cộng số điểm tích lũy mới vào số điểm hiện tại của khách hàng
                    $new_diem_tich_luy = $current_diem_tich_luy + $so_diem_tich_luy;

                    // Trừ số điểm sử dụng nếu có
                    $diem_tich_luy_su_dung = $diem_tich_luy_su_dung / 1000;
                    $new_diem_tich_luy -= $diem_tich_luy_su_dung;

                    $query_update_diem = "UPDATE khach_hang SET diem_tich_luy = $new_diem_tich_luy WHERE id = ?";
                    $this->db->update($query_update_diem, [$user_id]);
                }
            ...
            if ($resultA) {
                ...
                if ($diem_tich_luy_su_dung > 0) {
                    $user_id = $_SESSION['user_id'];

                    $query_get_diem = "SELECT diem_tich_luy FROM khach_hang WHERE id = '$user_id'";
                    $current_diem_tich_luy = $this->db->select($query_get_diem)->fetchColumn();

                    if ($current_diem_tich_luy !== false) {
                        $diem_tich_luy_su_dung = $diem_tich_luy_su_dung / 1000;
                        $new_diem_tich_luy = $current_diem_tich_luy - $diem_tich_luy_su_dung;

                        $query_update_diem = "UPDATE khach_hang SET diem_tich_luy = $new_diem_tich_luy WHERE id = '$user_id'";
                        $this->db->update($query_update_diem);
                    }
                }

                // Chèn vào bảng tbl_payment trong phần else
                $query = "INSERT INTO tbl_payment (session_idA, giaohang, thanhtoan, order_date) VALUES 
                  ('$session_idA', '$deliver_method', '$method_payment', '$today')";
                $result = $this->db->insert($query);
                var_dump($result);
